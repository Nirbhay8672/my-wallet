<template>
    <Modal :id="modalId" @close="handleClose">
        <template #modal_title>
            {{ isEdit ? 'Edit Income Type' : 'Add Income Type' }}
        </template>
...
function openModal(incomeType) {
    ...
    const image = document.getElementById("income_type_icon_file");
    if (image) {
        image.src = "https://thumbs.dreamstime.com/b/earn-money-vector-logo-icon-design-salary-symbol-hand-illustration-illustrations-152825364.jpg";
    }
    ...
    title_text.value = incomeType ? `Update Income Type : ${incomeType.name}` : "Create Income Type";
    button_text.value = incomeType ? "Update" : "Submit";

    // Populate fields if editing
    if (incomeType) {
        Object.assign(fields, incomeType);
        fields.icon_path = incomeType.icon;
        fields.icon = "";
        fields.active = incomeType.active !== undefined ? incomeType.active : true;
        ...
        if (incomeType.icon) {
            if (image) {
                image.src = incomeType.icon;
            }
        }
    } else {
        // Enable icon validation for new income types
...
function previewFiles(event) {
    ...
    if (file) {
        ...
        fields.icon = file;
        var image = document.getElementById("income_type_icon_file");
...
function clearFormData() {
    ...
    const image = document.getElementById("income_type_icon_file");
    if (image) {
        image.src = "https://www.pngplay.com/wp-content/uploads/6/Income-Icon-PNG-Image.png";
    }
...
function handleSubmit() {
    ...
    let url = props.isEdit ? `/income-types/update/${fields.id}` : "/income-types/create";
...
function handleStatusChange(newValue) {
    ...
    console.log('Income type status changed to:', newValue);
}
...
let formValidation = reactive(
    new FormValidation(fields, {
        icon_path: {
            required: "Income type icon is required.",
        },
        name: {
            required: "Income type name is required.",
        },
    })
);
</script>
