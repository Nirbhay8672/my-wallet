<template>
    <inertia-head title="Income Entries" />
    <main-page :page_name="'Income Entries'">
        <div class="d-flex justify-content-between align-items-center mb-1">
            ...
            <button class="btn btn-primary btn-sm" @click="editEntry(null)">
                <i class="fa fa-plus me-1"></i>Add Income Entry
            </button>
        </div>
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            <tr v-for="entry in entries.data" :key="entry.id">
                                ...
                                <td style="min-width: 120px">
                                    <span class="badge bg-success">{{ entry.income_type.name }}</span>
                                </td>
                                <td style="min-width: 120px">
                            ...
                            <tr v-if="!entries.data.length">
                                <td colspan="7" class="text-center align-middle">No income entries found</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
import IncomeEntryForm from './Form.vue';
...
const incomeEntryForm = ref(null);
const loader = ref(true);

const filters = reactive({
    account_id: props.filters?.account_id || '',
    income_type_id: props.filters?.income_type_id || '',
...
const editEntry = (entry) => {
    ...
    if (incomeEntryForm.value) {
        incomeEntryForm.value.openModal(entry);
    }
};
...
const deleteEntry = (entry) => {
    confirmAlert({
        ...
        if (result.isConfirmed) {
            axios.get(`/income-entries/delete/${entry.id}`)
                .then((response) => {
                ...
                .catch(function (error) {
                    if (error.response && error.response.status === 422) {
                        ...
                        toastAlert({
                            title: "Something went wrong while deleting the income entry.",
                            icon: "error",
                        });
                    }
                });
...
const applyFilters = () => {
    ...
    router.get('/income-entries/index', params);
};
